...
use crate::context::ResolvedForumContext;
use crate::AppState;
...
fn iam_strict() -> bool {
    matches!(
        std::env::var("SDKWORK_FORUM_IAM_STRICT").as_deref(),
        Ok("1") | Ok("true") | Ok("TRUE")
    )
}
        ...
        (Some(auth), Some(access)) => {
            match resolve_iam_app_context_from_dual_tokens(pool, &auth, &access).await {
                Some(iam) => {
                    request
                        .extensions_mut()
                        .insert(ResolvedForumContext(forum_context_from_iam(&iam)));
                }
...
mod tests {
    ...
    #[test]
    fn forum_context_from_iam_parses_numeric_ids() {
        ...
        let ctx = forum_context_from_iam(&iam);
