//! Forum backend-api gateway route manifest derived from the authored route descriptors.

use sdkwork_web_core::{HttpMethod, HttpRoute, HttpRouteManifest};

use crate::routes::{RouteDescriptor, BACKEND_ROUTES};
...
pub fn gateway_route_manifest() -> HttpRouteManifest {
    HttpRouteManifest::from_owned_routes(BACKEND_ROUTES.iter().map(http_route).collect())
}
