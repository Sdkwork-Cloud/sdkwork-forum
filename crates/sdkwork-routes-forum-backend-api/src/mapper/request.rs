...
pub fn extract_node_id_from_path(path: &str) -> Option<i64> {
    extract_path_param(path, 5).and_then(|s| s.parse().ok())
}
...
pub fn extract_case_id_from_path(path: &str) -> Option<i64> {
    extract_path_param(path, 6).and_then(|s| s.parse().ok())
}

pub fn extract_sanction_id_from_path(path: &str) -> Option<i64> {
    extract_path_param(path, 5).and_then(|s| s.parse().ok())
}
