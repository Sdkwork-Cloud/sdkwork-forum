> Migrated from `docs/adr/ADR-0001-forum-topic-reply-naming.md` on 2026-06-24.
...
Forum systems often use several names for the primary discussion unit. SDKWork needs names that are clear in APIs, database contracts, SDK resources, Rust modules, and documentation.

## Decision

Forum public contracts use `topic` and `reply` terminology.
...
- Tables use names such as `forum_topic` and `forum_topic_reply`.
