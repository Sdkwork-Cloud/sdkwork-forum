> Migrated from `docs/forum-api-design.md` on 2026-06-24.
...
- SDK family: `sdkwork-forum-app-sdk`.
...
- SDK family: `sdkwork-forum-backend-sdk`.
...
The repository layer stores idempotency records in `forum_idempotency_record` with:
...
- `tests/contract/forum-contract.test.mjs` - Validates route manifests, assemblies, auth modes
- `tests/schema/forum-schema.test.mjs` - Validates database schema completeness
- `tests/sdk/forum-sdk.test.mjs` - Validates sdkgen configs and composed facades
- `tests/static/forum-contract-boundary.test.mjs` - Validates forbidden terms and boundary rules
