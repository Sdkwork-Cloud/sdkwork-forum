...
2. Send notification via `ForumNotificationPort.publish_moderation_alert()`
3. Update queue item `severity` to next level
4. Log escalation event in `forum_audit_action`
...
5. Senior moderator reviews and decides:
   - `accepted` - Lift sanction, update `forum_sanction.lifted_at`
...
FROM forum_moderation_decision md
JOIN forum_moderation_case mc ON mc.id = md.case_id
...
FROM forum_sanction fs
LEFT JOIN forum_appeal fa ON fa.sanction_id = fs.id
