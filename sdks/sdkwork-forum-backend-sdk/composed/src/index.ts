export interface ForumBackendSdkDependencies {
  forum: ForumBackendGeneratedClient;
...
export interface ForumBackendGeneratedClient {
  nodes: {
    list(params?: { cursor?: string; limit?: number }): Promise<NodePage>;
    create(body: CreateNodeRequest): Promise<ForumNode>;
    update(nodeId: string, body: UpdateNodeRequest): Promise<ForumNode>;
    delete(nodeId: string): Promise<void>;
  };
  topicPrefixes: {
    list(params?: { boardId?: string; cursor?: string; limit?: number }): Promise<TopicPrefixPage>;
    create(body: CreateTopicPrefixRequest): Promise<ForumTopicPrefix>;
  };
  topics: {
  ...
  sanctions: {
    ...
    create(body: CreateSanctionRequest): Promise<ForumSanction>;
    update(sanctionId: string, body: UpdateSanctionRequest): Promise<ForumSanction>;
  };
  reputation: {
    rules: {
  ...
  badges: {
    ...
    create(body: CreateBadgeRequest): Promise<ForumBadge>;
  };
  stats: {
...
export interface ForumTopicPrefix {
  id: string;
  uuid: string;
  boardId: string;
  code: string;
...
export type NodePage = CursorPage<ForumNode>;
export type TopicPage = CursorPage<ForumTopic>;
export type TopicPrefixPage = CursorPage<ForumTopicPrefix>;
export type ModerationQueuePage = CursorPage<ModerationCase>;
export type ModerationCasePage = CursorPage<ModerationCase>;
export type SanctionPage = CursorPage<ForumSanction>;
...
export type BadgePage = CursorPage<ForumBadge>;
...
export class ForumBackendFacade {
  constructor(private readonly deps: ForumBackendSdkDependencies) {}

  async createModerationDecision(caseId: string, body: CreateDecisionRequest): Promise<ModerationDecision> {
    const decision = await this.deps.forum.moderation.cases.decisions.create(caseId, body);
  ...
  async rebuildSearchProjection(body?: { scope?: string; boardId?: string }): Promise<void> {
    await this.deps.forum.search.reindex.create(body);
    await this.deps.search.rebuildIndex(body?.boardId);
  }

  async listNodes(params?: { cursor?: string; limit?: number }): Promise<NodePage> {
    return this.deps.forum.nodes.list(params);
  }

  async createNode(body: CreateNodeRequest): Promise<ForumNode> {
    return this.deps.forum.nodes.create(body);
  }

  async updateNode(nodeId: string, body: UpdateNodeRequest): Promise<ForumNode> {
    return this.deps.forum.nodes.update(nodeId, body);
  }

  async deleteNode(nodeId: string): Promise<void> {
    return this.deps.forum.nodes.delete(nodeId);
  }

  async listTopicPrefixes(params?: { boardId?: string; cursor?: string; limit?: number }): Promise<TopicPrefixPage> {
    return this.deps.forum.topicPrefixes.list(params);
  }

  async createTopicPrefix(body: CreateTopicPrefixRequest): Promise<ForumTopicPrefix> {
    return this.deps.forum.topicPrefixes.create(body);
  }

  async listTopics(params?: { cursor?: string; limit?: number }): Promise<TopicPage> {
    return this.deps.forum.topics.list(params);
  }
